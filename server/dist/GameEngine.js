...
class GameEngine {
    constructor(roomId, onStateChange) {
        this.deck = [];
        this.turnTimer = null;
        this.TURN_DURATION_MS = 30000; // 30 seconds
    ...
    // --- Lobby Management ---
    addPlayer(socketId, sessionId, name) {
    ...
    handleTurnTimeout() {
        ...
        if (this.state.phase === 'BIDDING') {
            ...
            const validCard = player.hand.find(c => {
                ...
                if (hasLead)
                    return c.suit === this.state.leadSuit;
                return true;
            });
            if (validCard) {
    ...
    addChatMessage(sender, message) {
        ...
        if (this.state.chatLog.length > 50)
